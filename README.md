...
Free-tier models: `gemini-2.0-flash`, `gemini-2.5-flash`. Keep `--concurrency`
low (≈4) for rate limits; runs resume automatically if interrupted (re-run the
same command — completed `sample_id`s are skipped).
...
`OpenDriveLab/DriveLM` (gated: auto). One-time: make a free HF account, click
**Agree to access** at <https://huggingface.co/datasets/OpenDriveLab/DriveLM>,
create a token, then `export HF_TOKEN=hf_...`. Fetch only your subset's images
(range requests — no 705 MB download):

```bash
python3 scripts/fetch_images.py --qa-file data/raw/drivebench/drivebench-test.json \
    --frames 15 --split val --out-root data/raw/drivebench
```
